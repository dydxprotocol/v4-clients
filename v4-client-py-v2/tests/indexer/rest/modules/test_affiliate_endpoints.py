...
async def test_get_metadata(indexer_rest_client, test_address):
    ...
    assert response["referralCode"] == "GreenChain2I1"


@pytest.mark.asyncio
@retry_on_forbidden(max_retries=3, delay=1, skip=True)
async def test_get_address(indexer_rest_client, test_address):
    response = await indexer_rest_client.affiliate.get_address("GreenChain2I1")
    assert response is not None
...
async def test_get_snapshot(indexer_rest_client, test_address):
    addresses = [
        None,
        "dydx1s49hc95ggnl6fcq328rw35qmfx05a8zllp9wza",
        "dydx1s49hc95ggnl6fcq328rw35qmfx05a8zllp9wza,dydx13hmhafzju4r3z2pjtqzezha90fvy3ljhqyq49h",
    ]
    ...
    for address in addresses:
        for sort_by_affiliate_earning in sort_by_affiliate_earnings:
            for limit in limits:
                response = await indexer_rest_client.affiliate.get_snapshot(
                    address, sort_by_affiliate_earning, limit
                )
                assert response is not None
