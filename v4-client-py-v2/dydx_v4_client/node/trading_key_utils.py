...
def get_authorize_new_trading_key_arguments(
    ...
    any_of_message_filter_config_b64 = wrap_and_encode64(
        json.dumps(message_filter_sub_auth)
    )
    ...
    for auth in account_authenticators:
        ...
        try:
            ...
            for sub_auth in parsed_config:
                if (
                    isinstance(sub_auth, dict)
                    and sub_auth.get("type") == AuthenticatorType.SignatureVerification
                ):
            ...
            try:
                ...
                address = bech32.bech32_encode(
                    "dydx", bech32.convertbits(ripemd160_hash, 8, 5)
                )
            except Exception:
                continue
