...
async def check_funding_account_balance(
    ...
    try:
        ...
        response = await node_client.get_account_balance(
            test_address, 
            Denom.USDC.value
        )
        ...
        if balance_amount >= min_balance_usdc:
            ...
            print(f"✅ Funding account has sufficient balance: {usdc_amount:.6f} USDC ({balance_amount} micro-USDC)")
            return True
        else:
            usdc_amount = balance_amount / 1_000_000
            required_usdc = min_balance_usdc / 1_000_000
            print(f"❌ Insufficient balance: {usdc_amount:.6f} USDC (required: {required_usdc:.6f} USDC)")
            return False
