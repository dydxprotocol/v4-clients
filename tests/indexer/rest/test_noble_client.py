...
@pytest.mark.asyncio
async def test_get_account_balances(noble_client, test_address):
    balances = await noble_client.get_account_balances(test_address)
    assert isinstance(balances, list)
    assert all(isinstance(balance, dict) for balance in balances)


@pytest.mark.asyncio
async def test_get_account_balance(noble_client, test_address):
    balance = await noble_client.get_account_balance(test_address, "usdc")
    assert isinstance(balance, dict)
    assert "usdc" in balance["denom"]


@pytest.mark.asyncio
async def test_ibc_transfer(noble_client):
    message = {
    ...
    tx_response = await noble_client.ibc_transfer([message])
...
@pytest.mark.asyncio
async def test_simulate_transaction(noble_client):
    message = {
        "depositor": noble_client.wallet.get_verifying_key().to_string(),
        "amount": {"denom": "usdc", "amount": "1000"},
    }
